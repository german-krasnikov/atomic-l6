    ...
    public void FixedUpdate(){
        ...
        if(Car_Speed_In_KPH < Maximum_Speed && Car_Started){ //if the car's current speed is less than the maximum speed
            ...
            foreach(WheelCollider Wheel in Back_Wheels){
                Wheel.motorTorque = this.ForwardDirection * ((Motor_Torque * 5)/(Back_Wheels.Count + Front_Wheels.Count));
            }
        }
        ...
        if(Car_Started){
            foreach(WheelCollider Wheel in Front_Wheels){
                Wheel.steerAngle = TurnDirection * Max_Steer_Angle; //Turn the wheels
            }
        }
        ...
        foreach(WheelCollider Wheel in Back_Wheels){
            ...
            if(wheelHit.sidewaysSlip < 0 )	
                tempo = (1 + -TurnDirection) * Mathf.Abs(wheelHit.sidewaysSlip *handBrakeFrictionMultiplier);
        ...
        if((this.ForwardDirection < 0) && Car_Started){ //Turn on the reverse lights when car is reversing
            //Turn on reverse light(s)
            Turn_On_ReverseLights();
        }

        if((this.ForwardDirection > 0) && Car_Started){
